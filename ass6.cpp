#include <iostream>
#include <climits>
using namespace std;

class UtilityGrid {
    int n;                   // number of stations
    int adj[20][20];         // adjacency matrix
    string station[20];      // station names

public:

    void createGraph() {
        cout << "Enter number of power stations: ";
        cin >> n;

        cout << "Enter station names:\n";
        for (int i = 0; i < n; i++)
            cin >> station[i];

        cout << "Enter adjacency matrix (time between stations):\n";
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                cin >> adj[i][j];
                if (adj[i][j] == 0 && i != j)
                    adj[i][j] = INT_MAX;  // no connection
            }
        }
    }

    //  NEW FUNCTION (Graph Display)
    void displayGraph() {
        cout << "\nAdjacency Matrix:\n";

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (adj[i][j] == INT_MAX)
                    cout << "INF ";
                else
                    cout << adj[i][j] << " ";
            }
            cout << endl;
        }

        cout << "\nGraph Representation (Connections):\n";

        for (int i = 0; i < n; i++) {
            cout << station[i] << " -> ";
            for (int j = 0; j < n; j++) {
                if (adj[i][j] != INT_MAX && i != j) {
                    cout << station[j] << "(" << adj[i][j] << ") ";
                }
            }
            cout << endl;
        }
    }

    void dijkstra(int start) {
        int dist[20];
        bool visited[20];

        for (int i = 0; i < n; i++) {
            dist[i] = INT_MAX;
            visited[i] = false;
        }

        dist[start] = 0;

        for (int count = 0; count < n - 1; count++) {
            int min = INT_MAX, u;

            for (int i = 0; i < n; i++) {
                if (!visited[i] && dist[i] <= min) {
                    min = dist[i];
                    u = i;
                }
            }

            visited[u] = true;

            for (int v = 0; v < n; v++) {
                if (!visited[v] && adj[u][v] != INT_MAX &&
                    dist[u] + adj[u][v] < dist[v]) {
                    dist[v] = dist[u] + adj[u][v];
                }
            }
        }

        cout << "\nMinimum power transmission time:\n";
        for (int i = 0; i < n; i++) {
            cout << station[start] << " -> " << station[i]
                 << " = " << dist[i] << endl;
        }
    }

    void reachableStations(int start, int limit) {
        int dist[20];
        bool visited[20];

        for (int i = 0; i < n; i++) {
            dist[i] = INT_MAX;
            visited[i] = false;
        }

        dist[start] = 0;

        for (int count = 0; count < n - 1; count++) {
            int min = INT_MAX, u;

            for (int i = 0; i < n; i++) {
                if (!visited[i] && dist[i] <= min) {
                    min = dist[i];
                    u = i;
                }
            }

            visited[u] = true;

            for (int v = 0; v < n; v++) {
                if (!visited[v] && adj[u][v] != INT_MAX &&
                    dist[u] + adj[u][v] < dist[v]) {
                    dist[v] = dist[u] + adj[u][v];
                }
            }
        }

        cout << "\nStations reachable within time <= " << limit << ":\n";
        for (int i = 0; i < n; i++) {
            if (dist[i] <= limit) {
                cout << station[i] << " (time = " << dist[i] << ")" << endl;
            }
        }
    }
};

int main() {
    UtilityGrid g;
    int start, limit;

    g.createGraph();

    // NEW LINE (GRAPH DISPLAY)
    g.displayGraph();

    cout << "\nEnter source station index: ";
    cin >> start;

    g.dijkstra(start);

    cout << "\nEnter maintenance time limit: ";
    cin >> limit;

    g.reachableStations(start, limit);

    return 0;
}
